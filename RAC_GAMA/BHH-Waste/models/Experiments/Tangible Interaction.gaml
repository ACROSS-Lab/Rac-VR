...
global  {
	...
	reflex detect_interaction when: not CHOOSING_VILLAGE_FOR_POOL and stage = PLAYER_ACTION_TURN  and (gama.machine_time > (last_action_time + (1000.0 * delay_between_actions))){
		string result <- string(decodeQR(cam,image_width::image_height, false));
		if result = nil {
			ready_action <- true;
		}
